...
SocketSystem::SocketSystem()
{
	...
	if (iResult != 0) {
		Display::show("WSAStartup failed with error = " + toString(iResult), always);
	}
}
...
SocketSystem::~SocketSystem()
{
	...
	Display::show("Socket System cleaning up");
}
...
bool Socket::send(size_t bytes, byte* buffer)
{
	...
	while (bytesLeft > 0)
	{
		...
		if (socket_ == INVALID_SOCKET || bytesSent == 0 || bytesSent == SOCKET_ERROR)
			return false;
		bytesLeft -= bytesSent;
		pBuf += bytesSent;
	}
...
{
	Display::show("SocketConnecter instance destroyed");
}
...
bool SocketConnecter::connect(const std::string& ip, size_t port)
{
	...
	for (ptr = result; ptr != NULL; ptr = ptr->ai_next) {
		...
		if (iResult == SOCKET_ERROR) {
			...
			int error = WSAGetLastError();
			Display::show("WSAGetLastError returned " + toString(error));
			continue;
		}
		break;
	}
...
{
	Display::show("SocketListener instance destroyed");
}
...
bool SocketListener::bind()
{
	...
	for (auto pResult = result; pResult != NULL; pResult = pResult->ai_next)
	{
		...
		Display::show("server created ListenSocket");
...
bool SocketListener::listen()
{
	...
	Display::show("server TCP listening socket setup complete");
	return true;
}
...
void ClientHandler::operator()(Socket& socket_)
{
	while (true)
	{
		...
		if (command.size() == 0)
			break;
		Display::show("command: " + command);

		if (command == "TEST_STRING_HANDLING")
		{
			if (testStringHandling(socket_))
				Display::show("----String Handling test passed", always);
			else
				Display::show("----String Handling test failed", always);
			continue; // go back and get another command
		}
		if (command == "TEST_BUFFER_HANDLING")
		{
			if (testBufferHandling(socket_))
				Display::show("----Buffer Handling test passed", always);
			else
				Display::show("----Buffer Handling test failed", always);
...
{
	Display::title("String handling test on server");
	...
	while (true)
	{
		...
		if (str.size() > 0)
		{
			...
			//Display::show("bytes recvd at server: " + toString(str.size() + 1));

			Display::show("server recvd: " + str, always);

			if (socket_.sendString(str))
			{
				// statement below is left here for testing
				//Display::show("bytes sent by server: " + toString(str.size() + 1));
			}
			else
			{
				return false;
...
{
	Display::title("Buffer handling test on server");
	const int BufLen = 20;
	...
	while (true)
	{
		...
		// statement below is left here for testing
		//Display::show("bytes recvd at server: " + toString(BufLen));

		Display::show("server recvd: " + temp, always);

		if (socket_.send(BufLen, buffer))
		{
			// statement below is left here for testing
			//Display::show("bytes sent by server: " + toString(BufLen));
		}
		else
		{
			return false;
...
void clientTestStringHandling(Socket& si)
{
	...
	for (size_t i = 0; i < 5; ++i)
	{
		...
		si.sendString(text);
		Display::show("client sending string: " + text, always);
	}
	...
	if (si.waitForData(100, 10))
	{
		while (true)
		{
			...
			if (str.size() == 0 || str == "TEST_END")
				break;
			Display::show("client recvd: " + str, always);
		}
	}
	else
	{
		Display::show("client wait for server timed-out", always);
	}
}
...
void clientTestBufferHandling(Socket& si)
{
	...
	for (size_t i = 0; i < 5; ++i)
	{
		...
		Display::show("client sending buffer: " + std::string(buffer), always);
	}
...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		Display::title("Starting string test on client");
		clientTestStringHandling(si);
		Display::title("Starting buffer test on client");
